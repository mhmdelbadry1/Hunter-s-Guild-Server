...
function LandingPage() {
  ...
  const [serverAction, setServerAction] = useState(
    () => localStorage.getItem("serverAction") || "idle",
  );
  ...
  useEffect(() => {
    ...
    socketRef.current = io(API_ROOT || window.location.origin, {
      query: { token },
    });
  ...
  const fetchPublicIp = async () => {
    setIpLoading(true);
    try {
      const res = await axios.get(apiUrl("/public-ip"));
    ...
    try {
      ...
      if (currentAction && currentAction !== "idle") {
        ...
        if (currentAction === "starting" && isOnline && isReady) {
          console.log(
            "Recovery: Server started and ready, resetting action state to idle",
          );
          setServerAction("idle");
          localStorage.setItem("serverAction", "idle");
          localStorage.removeItem("serverActionTimestamp");
        ...
        // If we're "restarting" and server is online AND ready, reset to idle
          ...
          console.log(
            "Recovery: Server restarted and ready, resetting action state to idle",
          );
          setServerAction("idle");
          localStorage.setItem("serverAction", "idle");
          localStorage.removeItem("serverActionTimestamp");
        ...
        // If we're "stopping" and server is offline, reset to idle
          ...
          console.log(
            "Recovery: Server is offline, resetting action state to idle",
          );
          setServerAction("idle");
          localStorage.setItem("serverAction", "idle");
          localStorage.removeItem("serverActionTimestamp");
        ...
        if (actionTimestamp) {
          ...
          if (elapsed > 180000) {
            // 3 minutes
            console.log(
              "Recovery: Action stuck for too long, forcing reset to idle",
            );
            setServerAction("idle");
            localStorage.setItem("serverAction", "idle");
            localStorage.removeItem("serverActionTimestamp");
          ...
          console.log(
            "Recovery: No timestamp for action state, checking if stale...",
          );
          // Give a grace period - if server state matches expected end state, reset
          if (
            (currentAction === "starting" && isOnline) ||
            (currentAction === "restarting" && isOnline) ||
            (currentAction === "stopping" && isOffline)
          ) {
            console.log(
              "Recovery: Server state matches expected, resetting to idle",
            );
            setServerAction("idle");
            localStorage.setItem("serverAction", "idle");
          }
    ...
    try {
      ...
      try {
        ...
        if (finalStatus && !finalStatus.running) {
          ...
          showNotification(
            "Stop command sent, but server is slow to halt. Check logs.",
          );
        }
      }
    } catch (error) {
  ...
  const restartServer = async () => {
    ...
    try {
      ...
      try {
        ...
        console.log(
          "Server didn't report offline state, proceeding to wait for online...",
        );
      }
  ...
  const handleVersionChange = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        forgeVersion:
          selectedServerType === "forge" ? selectedForgeBuild : undefined,
      };
  ...
  const resetServer = async () => {
    ...
    try {
      ...
      showNotification(
        `✅ ${response.data.message}. Start the server to create a new world!`,
      );
      await fetchStatus();
    } catch (error) {
    ...
    <div className="landing-container">
      ...
      <header className="landing-header">
        ...
        <nav className="nav-links">
          <ul>
            ...
            <li>
              <a href="#server-files">Server Files</a>
            </li>
            <li>
              <a href="#help">Help</a>
            </li>
      ...
      >
        ...
        >
          ...
          <p className="hero-subtitle">
            Your exclusive control panel for our Hunter × Hunter Minecraft
            server.
            <br />
            Start the server and explore many features coming soon!
          </p>
          <div className="hero-buttons">
            <a href="#get-started" className="btn hero-cta">
              Get Started
            </a>
            <a href="#how-to-play" className="btn hero-secondary">
              How to use it?
            </a>
          </div>
          <div className="online-status">
            {publicIp ? (
              ...
              <p className="private-access-text">
                Private Access – For Friends Only
              </p>
            )}
          </div>
        </div>
            ...
            <div className="version-steps">
              ...
              {selectedServerType === "forge" && (
                <div className="step-group">
                  <label>
                    3. Forge Build{" "}
                    {selectedMcVersion ? `(for ${selectedMcVersion})` : ""}
                  </label>
                  <select
                  ...
                  {selectedMcVersion &&
                    availableForgeBuilds.length === 0 &&
                    !loading && (
                      <p className="hint-text warning">
                        No Forge builds found for {selectedMcVersion}. Try
                        another version.
                      </p>
                    )}
                </div>
            ...
            <div className="modal-actions">
              <button
                ...
                disabled={
                  loading ||
                  !selectedMcVersion ||
                  (selectedServerType === "forge" && !selectedForgeBuild)
                }
              >
                ...
                <div className="status-badge-container">
                  ...
                  {serverInfo.running && (
                    <div className="status-meta">
                      <span>
                        {serverInfo.config?.serverType
                          ? serverInfo.config.serverType.toUpperCase()
                          : "SERVER"}{" "}
                        {serverInfo.config?.mcVersion || "Unknown"}
                      </span>
                      {serverInfo.players && (
                        <>
                          <span>•</span>
                          <span>
                            Players: {serverInfo.players.online}/
                      ...
                      {serverInfo.uptime !== null &&
                        serverInfo.uptime !== undefined && (
                          <>
                            <span>•</span>
                            <span>
                              Uptime: {Math.floor(serverInfo.uptime / 3600)}h{" "}
                              {Math.floor((serverInfo.uptime % 3600) / 60)}m
                            </span>
                          </>
                        )}
                    </div>
            ...
            <div className="control-group power-group">
              ...
              <div className="group-buttons">
                <button
                  ...
                  onClick={startServer}
                  disabled={
                    loading || serverAction !== "idle" || serverInfo?.running
                  }
                >
                  <svg
                    width="18"
            ...
            <div className="group-buttons">
              <button
                ...
                disabled={
                  loading ||
                  serverAction !== "idle" ||
                  (serverInfo && serverInfo.running)
                }
                title={
                  serverInfo && serverInfo.running
                    ? "Stop server to manage mods"
                    : "Manage server mods"
                }
              >
                <svg
            ...
            <div className="group-buttons">
              <button
                ...
                disabled={
                  loading ||
                  serverAction !== "idle" ||
                  paramServerOffline(serverInfo)
                }
                title={
                  paramServerOffline(serverInfo)
                    ? "Start server to backup"
                    : "Backup world data"
                }
              >
                <svg
                    ...
                    onClick={() => {
                      if (window.confirm(`Kick ${p.name}?`)) {
                        ...
                        axios
                          ...
                          .catch((err) =>
                            showNotification("Failed to kick: " + err.message),
                          );
                      }
                    }}
                  >
      ...
      <section className="connect-section">
        ...
        <p className="connect-subtitle">
          Join your fellow hunters and share your passion for gaming.
        </p>
      ...
      <footer className="landing-footer">
        <p>
          © 2025 Hunter's Guild. All rights reserved. | v1.2 (Updated{" "}
